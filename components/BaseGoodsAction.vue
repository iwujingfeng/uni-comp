	...
	<view class="flex a-center container">
		<view class="flex icon">
			<view class="flex f-col a-center icon-item" v-for="(item,index) in options" :key="index" @click="handleClick(item, index)">
				<view>
					<image class="icon-wrap" :src="item.iconPath" :style="{width:iconWidth}" mode="widthFix" v-if="item.iconPath"></image>
					<uni-icons :type="item.icon" :color="item.iconColor" v-else></uni-icons>
		...
		<view class="f-1 flex a-center btn" v-if="Array.isArray(buttons)">
			<view class="btn-icon flex a-center j-center" :class="{'f-btn' :!buttons[0].background}" :style="{background: buttons[0].background}"
			 @click="clickButton(buttons[0], 0)">
				{{buttons[0].text}}
			</view>
			<view class="btn-icon flex a-center j-center" :class="{'l-btn' :!buttons[1].background}" :style="{background: buttons[1].background}"
			 @click="clickButton(buttons[1], 1)">
				{{buttons[1].text}}
			</view>
		</view>
		<view class="f-1 flex a-center btn" v-else>
			<view class="btn-icon flex a-center j-center" :class="{'f-btn' :!buttons.background}" :style="{background: buttons.background}"
			 @click="clickButton(buttons, 0)">
				{{buttons.text}}
			</view>
		</view>
	</view>
